...
curl -O https://download.virtualbox.org/virtualbox/5.2.4/virtualbox-5.2_5.2.4-119785~Debian~stretch_amd64.deb
echo y | gdebi *.deb
rm -rf *.deb

curl -O https://download.virtualbox.org/virtualbox/5.2.4/Oracle_VM_VirtualBox_Extension_Pack-5.2.4.vbox-extpack
echo y | VBoxManage extpack install --replace Oracle_VM_VirtualBox_Extension_Pack-5.2.4.vbox-extpack
rm -f *.vbox-extpack
